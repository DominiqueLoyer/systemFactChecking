...
class CredibilityVerificationSystem:
    ...
    def calculate_overall_score(
        ...
        if reputation == 'High':
            adjustments += self.weights['reputation']
        elif reputation == 'Low':
            adjustments -= self.weights['reputation']
        total_weight += self.weights['reputation']
        
        # Domain age
        domain_age = rule_results['source_analysis'].get('domain_age_days')
        if domain_age is not None:
            if domain_age > 365 * 2:  # > 2 years
                adjustments += self.weights['domain_age']
            elif domain_age < 90:  # < 3 months
                adjustments -= self.weights['domain_age']
            total_weight += self.weights['domain_age']
        ...
        for fc in fact_checks:
            ...
            if rating in ['true', 'verified', 'correct']:
                adjustments += self.weights['fact_check'] / len(fact_checks)
            elif rating in ['false', 'fake', 'incorrect', 'needs verification']:
                adjustments -= self.weights['fact_check'] / len(fact_checks)
        if fact_checks:
            total_weight += self.weights['fact_check']
        
        # Linguistic markers
        ling = rule_results.get('linguistic_markers', {})
        certainty = ling.get('certainty', 0)
        doubt = ling.get('doubt', 0)
        sensationalism = ling.get('sensationalism', 0)
        
        if certainty > 0 and doubt == 0:
            adjustments += min(self.weights['linguistic'] * certainty, self.weights['linguistic'])
        if doubt > 0:
            adjustments -= min(self.weights['linguistic'] * doubt, self.weights['linguistic'])
        if sensationalism > 0:
            adjustments -= min(self.weights['linguistic'] * sensationalism * 0.5, self.weights['linguistic'])
        total_weight += self.weights['linguistic']
        
        # Sentiment
        sentiment = nlp_results.get('sentiment', {})
        if sentiment:
            if sentiment.get('label') == 'NEGATIVE' and sentiment.get('score', 0) > 0.85:
                adjustments -= self.weights['sentiment'] * 0.5
            total_weight += self.weights['sentiment']
        
        # Bias
        bias = nlp_results.get('bias_analysis', {})
        if bias and 'Flagged' in bias.get('label', ''):
            bias_score = bias.get('score', 0.5)
            adjustments -= self.weights['bias'] * ((bias_score - 0.5) * 2)
            total_weight += self.weights['bias']
        
        # Coherence
        coherence = nlp_results.get('coherence_score')
        if coherence is not None:
            adjustments += (coherence - 0.5) * self.weights['coherence']
            total_weight += self.weights['coherence']
        
        # Final score
        final_score = 0.5 + adjustments / total_weight if total_weight > 0 else 0.5
    ...
    def _get_score_factors(self, rule_results: Dict, nlp_results: Dict) -> List[Dict]:
        """Get list of factors that influenced the score."""
        factors = []
        ...
        age = rule_results['source_analysis'].get('domain_age_days')
        if age:
            factors.append({
                'factor': 'Domain Age',
                'value': f"{age} days",
                'impact': '+' if age > 730 else ('-' if age < 90 else '0')
            })
        
        for fc in rule_results.get('fact_checking', []):
            factors.append({
                'factor': 'Fact Check',
                'value': fc.get('rating', 'Unknown'),
                'impact': '+' if 'true' in fc.get('rating', '').lower() else '-'
            })
