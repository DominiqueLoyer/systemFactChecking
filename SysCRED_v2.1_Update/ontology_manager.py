...
class OntologyManager:
    ...
    CRED_NS = "http://www.dic9335.uqam.ca/ontologies/credibility-verification#"
    ...
    def add_evaluation_triplets(self, report: Dict[str, Any]) -> str:
        ...
        for i, fc in enumerate(fact_checks):
            ...
            self.data_graph.add((evidence_uri, RDF.type, self.cred.Evidence))
            self.data_graph.add((report_uri, self.cred.basedOnEvidence, evidence_uri))
            
            if fc.get('rating', '').lower() in ['true', 'verified', 'correct']:
                self.data_graph.add((evidence_uri, RDF.type, self.cred.SupportingEvidence))
            elif fc.get('rating', '').lower() in ['false', 'fake', 'incorrect']:
                self.data_graph.add((evidence_uri, RDF.type, self.cred.RefutingEvidence))
            
            self.data_graph.add((evidence_uri, self.cred.evidenceSnippet, 
